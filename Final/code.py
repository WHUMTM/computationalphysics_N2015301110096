# -*- coding: utf-8 -*-
"""
Created on Wed Dec 27 19:51:27 2017
...
class ISING(object):
    def __init__(self, _H_magnetic=0., _Temperature=0.5, _length=30):
...
class ENERGY_MAG(object):            # give the plot of energy and magnetization vs temperature
   def __init__(self):
        self.T=list(np.arange(0.5,5,0.1))
   def calculate(self):
        self.energy=[]
        self.mag=[]
        self.e_var=[]
        ...
        ax3=plt.subplot(223)
        ax4=plt.subplot(224)
        ax1.plot(self.T,self.mag,'o-m',markersize=5)
        ax2.plot(self.T,self.energy,'o-m',markersize=5)
        ax3.plot(self.T,self.mag_var,'o-m',markersize=5)
        ax4.plot(self.T,self.e_var,'o-m',markersize=5)
        plt.show()
cmp=ENERGY_MAG()
cmp.calculate()
